class SceneMenuController{
	...
	public sceneMenu(){
		...
		// space to start
		if(this.scene_chosen > 0) if (keys[32]) null; //initScene();
		// goto menu on m
		if (keys[77]) gameEngine.menuController.gotoMenu();
		// delete cookies with d
		if (keys[68]) gameEngine.levelMenuController.deleteAllStats();
	...
	public drawSceneIcon(scene:Scene){
	
		if(this.scene_chosen == scene.id) ctx.fillStyle = "lime";
		else ctx.fillStyle = "black";
		
		ctx.fillRect(scene.x, scene.y, 50, 50);
		
		if(scene.unlocked) ctx.fillStyle = "green";
